...
def skinMask(roi):
	...
	for i in range(0, x):
		for j in range(0, y):
			...
			if (abs(int(R[i][j]) - int(G[i][j])) > 15) and int((R[i][j]) > int(G[i][j])) and int((R[i][j]) > int(B[i][j])):
...
path = './' + 'images-like' + '/'
outPath = './' + 'images-like-res' + '/'
outPath1 = 'C:/Users/50493/Desktop/1/'
...
if __name__ == '__main__':
	...
	for i in range(len(fileList)):
		...
		roi, res = binaryMask(pic, 0, 0, 300, 300)
		cv2.imwrite(outPath + "1_" + str(i + 1) + ".png", res)
	'''
	fileList = os.listdir(outPath)
	for i in range(len(fileList)):
		pic = cv2.imread(outPath + fileList[i])
		cv2.imwrite(outPath1 + "1_" + str(i + 1) + ".png", pic)
'''