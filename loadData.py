...
path = './' + 'feature-like' + '/'
path_img = './' + 'images-like-res' + '/'

if __name__ == "__main__":
    # for i in range(1, 11):
    '''
    for j in range(1, 524):
        roi = cv2.imread(path_img + '1_' + str(j) + '.png')

        descirptor_in_use = np.absolute(fd.fourierDesciptor(roi))

        fd_name = path + '1_' + str(j) + '.txt'
...
'''
    for i in range(1, 530):
        roi = cv2.imread(path_img + '1_' + str(i) + '.png')
        img, d = fd.fourierDesciptor(roi)
        descirptor_in_use = abs(d)

        # print(descirptor_in_use[1])

        fd_name = path + '1_' + str(i) + '.txt'
        # fd_name = path + str(i) + '.txt'
        with open(fd_name, 'w', encoding='utf-8') as f:
            temp = descirptor_in_use[1]
            for k in range(1, len(descirptor_in_use)):
